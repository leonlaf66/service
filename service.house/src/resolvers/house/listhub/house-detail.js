 ...
 * 获取详情
 */
export default (data, rules, ctx, { includeRawValue }) => {
  const groups = rules.groups.group.map(group => {
    ...
    _.mapValues(group.items, (rule, field) => {
      ...
      if (opts.flat) { // 扁平item 转 items -> 特殊build
        ...
        items[field] = getFieldValue(data, field, opts, ctx, { includeRawValue })
      }
    })
 ...
 * 获取值
 */
function getFieldValue(data, field, opts, ctx, { includeRawValue }) {
  let entity = {}
