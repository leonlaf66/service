 ...
 * 获取详情
 */
export default (data, rules, referenceFields, ctx, { includeRawValue }) => {
  const groups = rules.groups.group.map(group => {
    ...
    let items = _.mapValues(group.items, (rule, field) => {
      ...
      return getFieldValue(data, field, opts, referenceFields, ctx.lang, { includeRawValue })
    })
 ...
 * 获取值
 */
function getFieldValue(data, field, opts, referenceFields, lang = 'zh-CN', { includeRawValue }) {
  let entity = {}
